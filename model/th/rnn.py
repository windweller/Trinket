...
class LogisticRegression(object):
    # multi-class normal logistic regression
    # Hope this works lol
    ...
    def __init__(self, inp, n_in, n_out):
        ...
        self.W, self.b = _linear_params(n_in, n_out, 'sm')
        E = T.dot(inp, self.W) + self.b
        probs = T.nnet.softmax(E)
        self.y_pred = T.argmax(probs, axis=1)
        self.out = probs
        # parameters of the model
        self.params = [self.W, self.b]


def cross_entropy(pred, targ, mask=None, normalize=False, return_seq=False):
    # only for LogisticRegression class
    out = -T.mean(T.log(pred)[T.arange(targ.shape[0]), targ])
    return out
...
class GRULayerAttention(GRULayer):
    ...
    def _step(self, x, dmask, prev_h, hs):
        ...
        weights = T.sum(self.phi_hs * gamma_h, axis=2,
                        keepdims=True)  # sum over hidden dimension (dot product of next_h with hs for each batch index), weights seq_length x batch_size x 1
